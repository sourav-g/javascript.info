//* Class is almost equivalent to creating a Constructor Fn. Hence, typeof class is also function.

class User {
  constructor(name) {
    this.name = name; //* Content of the constructor fn.
  }
  sayHi() {
...
//! But even fn constructer is called with new
...
//* Called in current context, so all 3 takes the correct button context
button.click(); // Button
button.blur(); // Button
button.hover(); // Button

//* Called in another context [ Taken from the CB queue and put to EC (done by event loop), once EC is empty ]
